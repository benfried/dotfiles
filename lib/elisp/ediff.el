Article 3351 of gnu.emacs.sources:
Path: news.columbia.edu!sol.ctr.columbia.edu!newsxfer.itd.umich.edu!gatech!udel!newsserv.cs.sunysb.edu!newsserv.cs.sunysb.edu!kifer
From: kifer@sbkifer.cs.sunysb.edu (Michael Kifer)
Newsgroups: gnu.emacs.sources
Subject: ediff.el v 1.44.3
Date: 15 Apr 1994 21:56:19 GMT
Organization: SUNY at Stony Brook
Lines: 3681
Distribution: world
Message-ID: <KIFER.94Apr15175621@sbkifer.cs.sunysb.edu>
NNTP-Posting-Host: sbkifer.cs.sunysb.edu


This is a slightly improved version of Ediff (with respect to the version I
posted two days ago). I fixed a few rough edges for people with limited
colormaps and fonts. Some minor bugs have also been fixed.
...
;;; ediff.el --- an Emacs 19 & Lemacs 19 visual interface to diff(1) & patch(1)
...
;; 15-Apr-94|1.44.3|~/packages/ediff.el.Z|
...
;; files compared via diff(1) and for patching files with patch(1).
...
;; The increase in startup speed is dramatic for large files with many
;; difference regions.
...
;; frame, Ediff will arrange to share a frame with the control
;; window. (If none of the files is visible, then both will share the
;; control window's frame.) The same algorithm works when you hit 'c'
...
;;  Ediff lets you run multiple sessions at once, i.e., you can invoke
...
;;  would have its own *ediff-control* buffer, as all the info regarding
;;  any particular session is local to the associated *ediff-control* buffer.
...
;;  Say, you want to compare three files pairwise---an arduous task. You
;;  may start by preparing three different frames, each with its own control
;;  window.  (This would require a very wide screen---I didn't say this is
;;  easy :-)
;;
...
;; flags from an Ediff buffer is to hit `h', switching to highlighting with
;; faces (unhighlighting on a dumb terminal).


;;; Remarks: 
;;  -------

;;  1. Ediff is heavily dependent on the goodies found only in Emacs 19
;;     (didn't test it with Emacs 19.X, where X < 19).
...
;;  	    	    	    	    	    	   using vc.el package
;;  	    	    	    	    	           (Emacs 19.22 and up).
;;  	    rcs-ediff	    	    	    	 - same as vc-ediff, but
;;  	    	    	    	    	    	   uses rcs.el; rcs.el 
;;  	    	    	    	    	    	   is not part of the
...
;;  (require 'ediff)
;;
;; in your .emacs file.  This way it is also easier to make changes to
;; the default Ediff setting, if such changes become necessary --- see
;; Customization.
;;
...
;; are used to emphasize fine differences between the current differences
...
;;
;; You may also want to take a look at how thees faces are defined in
;; Ediff. 
;;
...
;; `ediff-fine-diff-program' is diff(1), by default. However, you can use
;; any function as long as it produces output consistent with diff(1).
...
;;  2. Sometimes highlighting of fine diferences may disappear when you
;;  jump to a new difference region for which fine diffs have already been
;;  computed. Sometimes, this will come back after you recenter (hit `c'). 
;;  Sometimes they'll come back only after you cycle though various
;;  highlighting modes by hitting `h'. 
;;  The reason for this is that Emacs has a number of misterious bugs in
;;  regard with overlays. When highlighting of fine diffs turns suddenly
;;  off, you can try to put the cursor over one of the unhighlighted fine
;;  diffs and see that (overlays-at (point)) evaluates to nil, as if the
;;  overlays at this point have gone somewhere. However, evaluating
;;  (overlay-lists) shows that there are overlays at the cursor position.
;;  So, it is clearly an Emacs bug. Unfortunately, I can't reproduce this
;;  bug without Ediff, so reporting it is hard.
...
This program should produce output in the format of diff(1). One could
...
(emerge-defvar-local ediff-want-faces t
  "If t, differences will be highlighted using faces on a window system.
If nil, they will be highlighted using ASCII flags, ediff-before-flag
...
(emerge-defvar-local ediff-tmp-buffer nil
  "Temporary buffer used by Ediff for computing fine differences between
difference regions.")
...
(defun ediff-setup-windows (buffer-A buffer-B control-buffer &optional pos)
  ;; Make sure we are not in the minibuffer window when we are trying to
...
up an appropriate window config."
  ...
  (message
   "To resume Ediff, switch to buffer ``*ediff-control*'' and hit %S." key)))
...
;(message "m1: %S %S " ediff-A-buffer  (get-buffer-window control-buf))
;(message "m1: %S %S " ediff-A-buffer  (selected-window))
;(sit-for 2)
      (emerge-eval-in-buffer
       control-buf
      (ediff-refresh-mode-line)
...
(defun ediff-make-fine-diffs (&optional n flag)       
  ...
  (let ((current-diff-vector (aref ediff-difference-vector n))
	(file-A ediff-temp-file-A)
	(file-B ediff-temp-file-B))
	
    (cond ((and (eq flag 'noforce) (aref current-diff-vector 2)) 
	   nil)
	  ((eq flag 'skip)
	   (or (aref current-diff-vector 2)
	  ...
	  (t
	   ...
	   (mapcar (function (lambda (vec)
			       (ediff-delete-overlay (aref vec 0))
			       (ediff-delete-overlay (aref vec 1))))
		   (aref current-diff-vector 2))
...
(defun ediff-set-fine-diff-properties (n &optional default)
  (let ((fine-diff-vector (aref (aref ediff-difference-vector n) 2))
    ...
    (mapcar
     (function (lambda (vec)
		 (ediff-overlay-put (aref vec 0) 'face face-A)
		 (ediff-overlay-put (aref vec 0) 'priority priority-A)
		 
		 (ediff-overlay-put (aref vec 1) 'face face-B)
		 (ediff-overlay-put (aref vec 1) 'priority priority-B)
		 ))
     fine-diff-vector)
    ))
