From news.columbia.edu!sol.ctr.columbia.edu!samsung!uunet!dove!cme!durer!klm Fri Jun 19 09:34:30 EDT 1992
Article: 943 of gnu.emacs.sources
Path: news.columbia.edu!sol.ctr.columbia.edu!samsung!uunet!dove!cme!durer!klm
From: klm@cme.nist.gov (Ken Manheimer)
Newsgroups: gnu.emacs.sources
Subject: Extensive new outline mode - allout.el V 2.0
Message-ID: <KLM.92Jun14123428@glyph.cme.nist.gov>
Date: 14 Jun 92 16:34:28 GMT
Sender: news@cme.nist.gov
Distribution: gnu
Organization: National Institute of Standards and Technology
Lines: 2000
Nntp-Posting-Host: glyph
...
I've been using it regularly for a few months now, ironing out stray
bugs and intending to write up more extensive documentation.  I think
i've got most if not all of the significant bugs, but i'm inundated at
work and home, and doubt i'll have time to write up anything like an
info file or even a detailed outline of the new features, so you'll
have to make do for now with the 'outline-mode' and other function's
documentation strings, if you're interested in giving it a whirl.
...
;; A full-fledged outline mode, based on the original GNU emacs
;; outline functionality.  This version supercedes the original GNU
;; outline.el and my prior-released outext.el.  klm 9-Mar-1992
;;
...
(defun outline-ascend-to-depth (depth)
  ...
  (if (and (> depth 0)(<= depth (or (outline-depth) 0)))
      (let ((last-good (point)))
        (while (and (< depth (or (outline-depth) 0))
...
(defun outline-next-sibling (&optional backward)
  ...
  (if (and backward (bobp))
    ...
    (let ((start-depth (outline-depth))
      ...
      (if (and (not (eobp)) (= (outline-depth) start-depth))
          outline-recent-prefix-beginning
        (goto-char start-point)
        nil)
      )
...
(defun outline-sibling-index (&optional depth)
  ...
  (save-excursion
    (cond ((and depth (<= depth 0) 0))
          ((or (not depth) (= depth (or (outline-depth) 0)))
           (let ((index 1))
         ...
         (bullet-char
          ...
          (cond
           ...
           (outline-old-style-prefixes
            ...
            ;; bullet-char:
            (setq body (make-string depth outline-primary-bullet))
            (setq lead-char "")
            "")
...
(defun outline-rebullet-heading (&optional solicit
  ...
  (let* ((current-depth (or (outline-depth) 0))
