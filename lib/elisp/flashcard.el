Article 3556 of gnu.emacs.sources:
Path: news.columbia.edu!rutgers!concert!gatech!howland.reston.ans.net!cs.utexas.edu!csc.ti.com!tilde.csc.ti.com!m2.dseg.ti.com!ief!huttar
From: huttar@hp750.itg.ti.com (Lars Huttar)
Newsgroups: gnu.emacs.sources
Subject: flashcard.el: flexible drilling on flashcards, 0.95 beta
Message-ID: <HUTTAR.94Jun8101644@hp750.itg.ti.com>
Date: 8 Jun 94 15:16:43 GMT
Sender: usenet@ief.itg.ti.com (Mr. USENET)
Followup-To: gnu.emacs.sources
...
It's about time I posted this flashcard drilling package.  It's been
through some limited beta testing, and some people have found it
useful.  I use it for drilling on Japanese vocabulary.  It's a lot
more streamlined than using cardboard flashcards.  The best
explanation of what flashcard.el does is probably in the comments at
the beginning of the file.  Its salient features are:
  - drilling on files of flashcards, from one "field" to another
...
I will be leaving my place of computer access in about a week, and I
don't know if or when I'll have access after that.  So if you have
feedback for me, please don't hesitate.

I hope you find this package useful.

Lars Huttar
huttar@cs.oberlin.edu
Dallas, TX  USA
...
(if flashcard-mode-map nil
  ...
  (define-key flashcard-mode-map "q" 'fc-quit)
  (define-key flashcard-mode-map "?" 'describe-mode)
  (define-key flashcard-mode-map "h" 'describe-mode)
  (define-key flashcard-mode-map "o" 'fc-options)
  (define-key flashcard-mode-map "f" 'fc-load-flashcard-file)
  (define-key flashcard-mode-map "d" 'fc-start-drill)
  (define-key flashcard-mode-map "s" 'fc-save-scores)
  (define-key flashcard-mode-map "c" 'fc-clear-data)

  (define-key flashcard-mode-map " " 'fc-question-ready)
  (define-key flashcard-mode-map "g" 'fc-give-up)
  (define-key flashcard-mode-map "r" 'fc-answer-right)
  ;(define-key flashcard-mode-map "y" 'fc-answer-right)
  (define-key flashcard-mode-map "w" 'fc-answer-wrong)
  ;(define-key flashcard-mode-map "n" 'fc-answer-wrong)
  (define-key flashcard-mode-map "\C-m" 'fc-multi-proceed)
  (define-key flashcard-mode-map "k" 'fc-skip)
  (define-key flashcard-mode-map "u" 'fc-undo)

  (define-key flashcard-mode-map "0" 'fc-choose)
  (define-key flashcard-mode-map "1" 'fc-choose)
  (define-key flashcard-mode-map "2" 'fc-choose)
  (define-key flashcard-mode-map "3" 'fc-choose)
  (define-key flashcard-mode-map "4" 'fc-choose)
  (define-key flashcard-mode-map "5" 'fc-choose)
  (define-key flashcard-mode-map "6" 'fc-choose)
  (define-key flashcard-mode-map "7" 'fc-choose)
  (define-key flashcard-mode-map "8" 'fc-choose)
  (define-key flashcard-mode-map "9" 'fc-choose)
  (define-key flashcard-mode-map "-" 'fc-undefined)
...
;;               fc-question-ready, fc-give-up, or fc-skip
;;    answer-check - have displayed answer, waiting for user to hit
;;               fc-answer-right, fc-answer-wrong, or fc-skip.
...
(defvar fc-question-help
  "Press\t\\[fc-question-ready]\tif you know the answer,
\t\\[fc-give-up]\tto give up, or
...
(defvar fc-answer-help
  "Press\t\\[fc-answer-right]\tif you got it right,
\t\\[fc-answer-wrong]\tif you got it wrong, or
\t\\[fc-skip]\tto skip this question."
...
(defun fc-load-flashcard-file (file-name)
  (interactive
   ...
   (list
    (expand-file-name
     (read-file-name "Load flashcard file: " fc-flashcard-dir nil t))))
...
(defun fc-set-buffer-lines ()
  ...
  (setq fc-question-line 3              ; constant.
        fc-answer-line (+ fc-question-line 2
...
(defun fc-question-ready ()
  "User command called when user knows answer to question."
  (interactive)
...
(defun fc-answer-wrong ()
  "User command when user failed to get answer right."
  (interactive)
...
(defun fc-answer-right ()
  "User command when user got answer right."
  (interactive)
  (if (not (eq fc-state 'answer-check))
      (ding)
;;    (message "Working...")       ;; because fc-timestamp can take time,
;;                                 ;; especially if it is running Perl.  :-(
;;    (fc-sit-for 0)                      ;; Redisplay.
;;; Since we're no longer calling perl, let's try leaving out the message.
    (unwind-protect
        (progn
          (fc-check-score-field)
          (fc-backup-scores)
