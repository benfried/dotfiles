...
;; will make postscript-mode autoload this file, and cause postscript-mode
;; to be invoked automatically when visiting .ps or .cps files:
;; 
...
(defconst ps-postscript-command "psh"
  "*Command used to communicate with a printer or NeWS server")
...
(defun ps-indent-line nil
  ...
  (if (not (or (looking-at "%%")	; "%%" comments stay at left margin
	...
	(if (looking-at "\\(\\(dict\\|class\\)?end\\)\\|g?restore\\|closepath")
...
(defun ps-insert-d-char (arg)
  "Awful hack to make \"end\" keywords indent themselves."
  (interactive "p")
  (insert-char last-command-char arg)
  (save-excursion
    (beginning-of-line)
    (if (or
	 (looking-at "^[ \t]*\\(dict\\|class\\)?end")
	 (looking-at "^[ \t]*\\(g?restore\\|closepath\\)"))
	(progn
;	  (message "here - and yes")
	  (delete-horizontal-space)
	  (ps-indent-end)))))
...
character number of the character following `begin' or START if not found."
  (save-excursion
    (let ((depth 1) match)
      (while (and (> depth 0)
		  (re-search-backward
"^[ \t]*\\(\\(dict\\|class\\)?end\\|begin\\)\\|\\(g?save\\|restore\\)\\|newpath\\|closepath[ \t]*\\(%.*\\)*$" start t))
	(setq depth (if (looking-at "[ \t]*\\(\\(dict\\|class\\)?end\\)\\|g?restore\\|closepath")
			(1+ depth) (1- depth))))
      (if (not (eq 0 depth))
	  start
	(forward-word 1)
...
(if (null ps-mode-map)
    (progn
      ...
      (define-key ps-mode-map "e" 'ps-insert-d-char)
      (define-key ps-mode-map "h" 'ps-insert-d-char)
...
(defun ps-shell nil
  ...
  (make-local-variable shell-input-pattern "")
