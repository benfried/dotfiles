...
0,,
*** EOOH ***
Received: from topaz.rutgers.edu by cunixc.cc.columbia.edu (5.54/5.10) id AA02231; Tue, 8 Nov 88 17:46:47 EST
Received: by topaz.rutgers.edu (5.59/1.15) 
	id AA28124; Tue, 8 Nov 88 17:29:58 EST
Date: Tue, 8 Nov 88 17:29:58 EST
From: steiner@topaz.rutgers.edu (Dave Steiner)
Message-Id: <8811082229.AA28124@topaz.rutgers.edu>
To: luken@spot.cc.lehigh.edu, fritzson@prc.unisys.com, mlijews@nswc-wo.arpa,
        ben@cunixc.cc.columbia.edu, jsol@bu-it.bu.edu,
        kdw1@sphinx.uchicago.edu, gordon@hci.heriot-watt.ac.uk,
        mdb@silvlis.com
Subject: GNUEmacs rmail digestifier
Status: RO


These are the files to run the digestifying program.  The first is
...
They include summary-by-{senders,subject,date,to}, rmail-resend,
multiple delete of messages, and multiple output of messages.  You
will definitely need the patches to mail-utils.el.
...
;@@@ these should be commented out when a new emacs is built.
...
(defvar digest-to-address nil
  "*If non-nil, use this address for the To: field, instead of the
digest-name.")
...
(defun digest-move-messages (messages)
  ...
  (let ((foo messages)
	...
	(mail-default-reply-to
	 (concat digest-name "@" (capitalize (system-name)))))
...
(defvar digest-to-address nil
  "*If non-nil, use this address for the To: field, instead of the
digest-name.  NOTE: if non-nil, it should be a string!")
==================================================info/digest
-*- Text -*-
