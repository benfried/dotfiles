...
;; Version: $Header: mime.el,v 1.60 93/08/21 19:40:55 umerin Locked $
...
;; multilingual text written in Mule is declared as `X-ISO-2022-JP-2'
;; and such messages are encoded in Mule style.  Mule is required for
;; reading the messages of `X-ISO-2022-JP-2' charset.  As it contains
;; leading `X-', `X-ISO-2022-JP-2' charset is still experimental.  For
;; more information on Mule, please make a contact to mule@etl.go.jp
;; or mule-jp@etl.go.jp.
...
;; $Date: 93/08/21 19:40:55 $|$Revision: 1.60 $|~/misc/mime.el.Z|
...
Mule:		US-ASCII, ISO-8859-* (except for ISO-8859-6),
		ISO-2022-JP, and X-ISO-2022-JP-2 are available.

X-ISO-2022-JP-2 charset used in Mule is experimental and expected to
be used to represent multilingual text in intermixed manner.  Any
languages that has no registered charset are also represented as
X-ISO-2022-JP-2 in Mule.
...
(defun mime-test-content-type (contype type &optional subtype)
  ...
  (and (stringp contype)
       (string-match
	(concat "^[ \t]*" type "/" (or subtype "")) contype)))
...
(defun mime-header-charset-chooser-for-mule (begin end)
  "Return cons of charset and header encoding for a given header region.
Encoding name must be a short name, such as `B' or `Q'.  US-ASCII,
ISO-8859-* (except for ISO-8859-6), ISO-2022-JP and X-ISO-2022-JP-2
are supported on Mule."
...
(defun mime-body-charset-chooser-for-mule (begin end)
  "Return a cons of charset and encoding of a message in a given region.
Encoding name must be a canonical name, such as `base64'.  US-ASCII,
ISO-8859-* (except for ISO-8859-6), ISO-2022-JP and X-ISO-2022-JP-2
are supported on Mule.  Charset X-ISO-2022-JP-2 is used for
multilingual text, and it is expected to be encoded in Mule style."
  (let ((lclist (find-charset-region begin end)))
    (cond ((null lclist)
	   '("US-ASCII" . nil))		;Default charset of MIME.
...
X-ISO-2022-JP-2 are supported on Mule.  Charset X-ISO-2022-JP-2 used
for multilingual text is encoded in Mule style."
  (let* ((charset (car method))
	 (encoding (cdr method))
	 (coding-system
